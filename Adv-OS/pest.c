...
off_t offset;
int whence, fd, reverse;
char buffer[BUFFER_SIZE];

ssize_t read_next_block()
{
    ssize_t bytes;
    ...
    if(bytes < 0)
    {
        fprintf(stderr, "ERROR: Failed to read file block\n");
        exit(EXIT_FAILURE);
    }
...
{
        int line_count = 0, line_max = 0;
        size_t d_buffer_size = 0;
        char *line, *file_path, *str_ptr, *d_buffer = NULL;
        ssize_t bytes;
       ...
       // line_max = atoi(argv[2]);
        ...
        reverse = 1;
        whence = SEEK_CUR;
        offset = (reverse * BUFFER_SIZE);

        while(read_next_block() != 0)
            printf("%s", buffer);
